...
class Place(object):
        ...
        if insect.is_ant:
            if self.ant is None:
                if isinstance(insect, QueenAnt):    
                    if not QueenAnt.true_queen_placed:
                       QueenAnt.true_queen_placed = True 
                       self.ant = insect
                self.ant = insect
                #self.ant = insect
            else:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            if insect.is_queenant:
                return
            elif not insect.is_queenant:
                insect.place = None
            # END Problem 13

            # Special handling for container ants
        #if insect.name != 'Queen' or (insect.name == 'Queen' and insect.not_true_queen):
         #   if insect.is_ant:
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 7
        ant = self.place.ant 
        if not ant or ant.blocks_path == False:
            return False 
        return True 
        # END Problem 7
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        curr_place, go = self.place, 0
        while curr_place != hive:
            if curr_place.bees and go <= self.max_range and go >= self.min_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        bee_list = current_place.bees[:]
        # revised --> deleted the redudant code of the reduce_armor method from the Ant Class 
        # and replaced it with a call to that method (improves the code's modularity (implementing abstraction))
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting:
            ...
            if this_place.bees:
                self.eat_bee(random_or_none(this_place.bees))
...
class BodyguardAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 9
        ant = self.contained_ant
        if ant: 
            self.action = ant.action(colony)
...
# END Problem 13
    ...
    #is_watersafe = True
    food_cost = 7
    #not_true_queen = False 
    #ants = []
    #no_of_queen = 0 
    is_queenant = True
    true_queen_placed = False
    ...
    def __init__(self, armor=1):
        ...
        if not self.true_queen_placed:
            QueenAnt.true_queen_placed = True
        else:
            self.is_queenant = False

        #self.armor = armor 
        #QueenAnt.no_of_queen += 1
        #if QueenAnt.no_of_queen > 1:
         #   self.not_true_queen = True  
        # END Problem 13
    ...
    def action(self, colony):
        ...
        if not self.is_queenant:
            ...
            #initializing place to start indexing from as the true Queen's place
            place = self.place.exit
            while place:           # while loop breaks when var place is a none value; no additional operators needed
                if place.ant:
                    if not place.ant.buffed:
                        Ant.double_damage(place.ant)
                        place.ant.buffed = True
                    if place.ant.is_container and place.ant.contained_ant:  #Notes: place.ant.can_contain is wrong and inconvenient because a function is a Truth-y value and will always evaluate to True;                                                     #it's not practical to include it inside the If as it must be called on an other ant, and we don't know the                                                                             #the other ant (it's probably possible to determine, but unneccessarily difficult) because the can_contain method was made for the add_insect method
                        if not place.ant.contained_ant.buffed:
                            Ant.double_damage(place.ant.contained_ant)
                            place.ant.contained_ant.buffed = True
                place = place.exit

        # if not self.is_queenant:
        #     current = self.place
        #     while current.exit != None:
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 13
        Ant.reduce_armor(self, amount)              #method reduce_armor inherited from ThrowerAnt
        if not self.armor and self.is_queenant:          #if armor is 0 of the Queen_Ant, the if statement Falses
            bees_win()

        # self.armor -= amount
        # if not self.not_true_queen and self.armor <= 0:
        #     self.place.remove_insect(self)
        #     bees_win()
        # END Problem 13
...
def apply_effect(effect, bee, duration):
    ...
    new_action = effect(original_action, bee)

    def helper(colony):
        nonlocal duration
        if duration == 0:
            bee.direction = 'forward'
...
class ScaryThrower(ThrowerAnt):
    ...
    # BEGIN Problem EC
    implemented = False 
    food_cost = 6 # Change to True to view in the GUI
    # END Problem EC

    def throw_at(self, target):
        # BEGIN Problem EC
        if not target.scared_already:
            return apply_effect(make_scare, target, 2)
        return None
        # END Problem EC
